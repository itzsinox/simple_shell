...
{
int x = 0;
char *y = NULL; list_t *n = NULL;
if (info->argc == 1)
{
n = info->alias; while (n)
{
print_alias(n); n = n->next;
}
return (0);
}
for (x = 1; info->argv[x]; x++)
{
y = _strchr(info->argv[x], '='); if (y)
set_alias(info, info->argv[x]); else
